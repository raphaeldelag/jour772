...
```{r}
# CHICAGO ANALYSIS
# Get Chicago homicides and arrests by year before 2015
num_hom_and_arrest_chicago_by_year_pre_2015 <- homicides_wapo %>%
  filter(city == "Chicago" & reported_date < "2015-01-01") %>%
  mutate(year = year(reported_date)) %>%
  group_by(year) %>%
  summarise(
    homicides = n(),
...
pop_estimates_chicago_post_2015 <- get_estimates(
    ...
    time_series = TRUE,
    state = "IL",
    place = "14000",  # Chicago place code
    variable = "POP"
  ) %>%
  mutate(year = case_when(
    DATE == 1 ~ 2010,
...
```{r}
# Calculate pre-2015 rates
chi_hom_rates_pre_2015 <- num_hom_and_arrest_chicago_by_year_pre_2015 %>%
  left_join(pop_estimates_chicago_pre_2015, by = "year") %>%
  mutate(homicide_rate_per_100k = round((homicides / population) * 100000, 1))
```
...
```{r}
# Combine all years into one comparison
chi_rates_all_years <- bind_rows(
  chi_hom_rates_pre_2015,
  chi_hom_rates_post_2015
) %>%
  arrange(year)
```{r}
# Combine all years into one comparison
