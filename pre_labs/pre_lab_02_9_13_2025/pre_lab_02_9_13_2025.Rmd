...
**Answer**
The new variable, selected_journalism_courses, will allow us to work with the neccesary columns that are of interest to our analysis while also presevering the original data. Any analysis we may need to do with the original data frome will be able to be done in a more organized and accurate way. 
...
**Answer:**
The line,"mutate(expenditure_date=mdy(expenditure_date))," updates the column to a "<date>" from a character column. The lubridate package allows this function.
...
**Answer**
The new "month" column groups all entries from the same month, marking each with the first day of that month using the line 'floor_date(expenditure_date, "month")'. This allows us to then sum the expenses by month.
...
**Answer**
Using the data from the "general_22" data frame, which was created by importing the "md_gov_county.csv" file, we are adding two new columns. The first column, "total_votes," calculates the sum of all votes for each candidate. The second column, "pct_moore," shows the percentage of those total votes that went to the candidate named Moore.
...
**Answer**
The "arrange(desc(pct_moore))" sorts the results withe highest percentage of votes for Moore appear at the top. We see these percentage as whole numbers by multiplying the result from "mutate(pct_moore = (moore/total_votes)" by 100 in order to no longer have a decimal in the "pct_moore" columns but instead as a whole number percentage.
...
**Answer**
Well this would allow us to see where in Maryland voters voted less for candidate Moore which can be helpful f we are looking to see where in the state Moore has less support. 
...
**Answer**
Perplexity question:what tidyverse functions are similar to str_to_upper and explain what they do

What I understood: Two similar functions are "str_to_lower()" and "str_to_title()" which both manipulate and standardize the character data in "maryland_expense." The first converts all the characters in a string (i.e. a word/name/etc) into lowercase text. The latter converts the first letter of each word in a string to uppercase and the rest of the letters to lowercase.
...
**Answer**
We are adding a new column called "state" to the "maryland_expenses_with_state" data frame. The code groups checks the address for a state name abbreviation and calculates the total expenditure for each state. The results are then sorted from highest to lowest expenditure. 
...
1. How have the total expenditures by Maryland political committees changed over time when grouped by month. Which months stand out with unusually high or low spending?


2.  Which UMD journalism courses have the highest average enrollment? How does that ranking change when you factor in semester-over-semester growth?

3.  How do total expenditures vary by state over time, and which states show trends of increasing or decreasing spending throughout the year?